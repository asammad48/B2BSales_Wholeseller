  ...
  return (
    <div ref={containerRef} className={`space-y-3 ${className || ''}`}>
      <button
        ...
        disabled={disabled}
        onClick={() => setIsOpen((prev) => !prev)}
        className="relative w-full bg-gray-50 border-none rounded-xl py-3 px-4 outline-none focus:ring-2 focus:ring-gray-200 transition-all text-left disabled:opacity-50"
      >
      ...
      {selectedOptions.length ? (
        <div className="space-y-2">
          {selectedOptions.map((option) => (
            <div key={option.value} className="flex items-start justify-between gap-3 rounded-2xl bg-white border border-gray-100 px-4 py-3">
              <div>
      ...
      {isOpen && (
        <div className="z-30 rounded-2xl bg-white shadow-lg border border-gray-100 overflow-hidden">
          <div className="p-3 border-b border-gray-100 bg-white">
            <div className="relative">
              <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
              <input
              ...
              filteredOptions.map((option, index) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full rounded-xl px-3 py-2.5 text-left text-sm transition-colors ${
                      isHighlighted ? 'bg-gray-50 text-gray-900' : 'text-gray-600 hover:bg-gray-50'
                    }`}
                  >
                    {option.label}
                  </button>
